...
redox_fit <- function(df, f, method = c("relative", "absolute"), regression = TRUE) {
  ...
  # Defensive column check for clearer errors
  ...
  # --- Reduction step (use bare column names in data.table expressions) ---
  df$Fo <- rep(
    df[DataPt %in% 1, mean(EM), by = .(Target_ID, TimeSec)]$V1,
    each = f + 1
  )
  df$Fm <- rep(
    df[DataPt %in% 298:302, max(EM), by = .(Target_ID, TimeSec)]$V1,
    each = f + 1
  )
  ...
  for (m in method) {
    if (m == "relative") {
      # Keller method: use Relative
      df$Fr1.Area <- rep(
        df[DataPt %in% 303:320, agricolae::audpc(Relative, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr1.Time <-
        rep(df[DataPt %in% 320, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1) -
        rep(df[DataPt %in% 303, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1)
      df$Fr1.Fq <- 1 - (df$Fr1.Area / df$Fr1.Time)

      df$Fr2.Area <- rep(
        df[DataPt %in% 303:360, agricolae::audpc(Relative, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr2.Time <-
        rep(df[DataPt %in% 360, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1) -
        rep(df[DataPt %in% 303, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1)
      df$Fr2.Fq <- 1 - (df$Fr2.Area / df$Fr2.Time)

      df$Fr.Area <- rep(
        df[DataPt %in% 303:f, agricolae::audpc(Relative, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr.Time <-
        rep(df[DataPt %in% f, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1) -
        rep(df[DataPt %in% 303, mean(Time.us), by = .(Target_ID, TimeSec)]$V1, each = f + 1)
      df$Fr.Fq <- 1 - (df$Fr.Area / df$Fr.Time)

    } else if (m == "absolute") {
      # Zendonadi method: use EM and normalize by Fm
      df$Fr1.Area <- rep(
        df[DataPt %in% 303:320, agricolae::audpc(EM, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr1.Time <-
      ...
      df$Fr2.Area <- rep(
        df[DataPt %in% 303:360, agricolae::audpc(EM, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr2.Time <-
      ...
      df$Fr.Area <- rep(
        df[DataPt %in% 303:f, agricolae::audpc(EM, Time.us, type = "absolute"),
           by = .(Target_ID, TimeSec)]$V1,
        each = f + 1
      )
      df$Fr.Time <-
